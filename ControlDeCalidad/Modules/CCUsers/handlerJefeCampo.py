...
class ManagerUserSystem(object):
	...
	def getInformationUser(self):
		...
		query = "select jefeCampo.nombreJefeCampo, campo.nombreCampo, telefono.numeroTelefono, jefeCampo.correoJefeCampo  from jefeCampo join campo on (jefeCampo.campoPertenece = campo.idcampo) join telefono on (jefeCampo.telefono = telefono.idtelefono)"
	...
	def insertJefeCampo(self, numero, empresa, nombreJefe, nombreCampo, correo):
		...
		query = "insert into jefeCampo (jefeCampo.idjefeCampo, jefeCampo.nombreJefeCampo, jefeCampo.telefono, jefeCampo.campoPertenece, jefeCampo.correoJefeCampo) values (%d, '%s', %d, %d, '%s')" % (int(idJefeCampo), str(nombreJefe), int(idTelefono), int(idCampo), str(correo))
	...
	def editUserSystem(self, oldjefeCampo, oldcampo, oldtelefono, oldCorreo, jefeCampo, campo, telefono, empresa, correo):
		...
		self.Connect.initConnectionDB()
		query = "update jefeCampo set jefeCampo.nombreJefeCampo = \"%s\", jefeCampo.campoPertenece = %d, jefeCampo.telefono = %d, jefeCampo.correoJefeCampo = '%s' where jefeCampo.idjefeCampo = %d" % (str(jefeCampo), int(idCampo), int (idTelefono), str (correo), int (idJefeCampoEdit))
		self.CrudDataBase.insertToTable(query, self.Connect)
		self.Connect.closeConnectionDB()

	#delete user of system...
	def deleteUserSystem(self, jefeCampo, campo, telefono, correo):
		
		jefeCampo = jefeCampo[:len(jefeCampo)-1]
		jefeCampo = jefeCampo.replace("-", " ")
		...
		#get id user for will be edit...
		query = "select jefeCampo.idjefeCampo from jefeCampo join campo on (campo.idcampo = jefeCampo.campoPertenece) join telefono on (telefono.idtelefono = jefeCampo.telefono) where jefeCampo.nombreJefeCampo =\"%s\" AND campo.nombreCampo = \"%s\" AND telefono.numeroTelefono = %d AND jefeCampo.correoJefeCampo = \"%s\"" % (str(jefeCampo), str(campo), int(telefono), str(correo))
...
def main ():
	...
	if sys.argv[1] == "1":#get information of user
		...
		st.insertJefeCampo(sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6])
	
	elif sys.argv[1] == "4":
		st.editUserSystem(sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], sys.argv[9], sys.argv[10])
